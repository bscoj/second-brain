      ...
      >
        {effectiveOpen && (
          <Link
            ...
            className="flex flex-col items-start overflow-hidden px-1"
          >
            <span className="second-brain-title text-[1.05rem] font-semibold text-[#f3e8d7]">
              Second Brain
            </span>
            <span className="text-[10px] uppercase tracking-[0.28em] text-[#b39d82]">
              Research Companion
            </span>
          </Link>
        )}
          ...
          <SidebarMenuItem>
            <Tooltip>
              <TooltipTrigger asChild>
                <SidebarMenuButton
                  type="button"
                  className="second-brain-panel h-10 rounded-2xl p-1 transition-colors hover:bg-white/[0.07] md:p-2 cursor-pointer"
                  onClick={() => {
                    setOpenMobile(false);
                    navigate('/');
                  }}
              ...
              <TooltipContent side="right" style={{ display: open ? 'none' : 'block' }}>New thread</TooltipContent>
            </Tooltip>
          </SidebarMenuItem>
        </SidebarMenu>
      </div>
