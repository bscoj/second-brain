...
export function Chat({
  ...
  if (messages.length === 0) {
    return (
      <div className="flex h-dvh min-w-0 flex-col bg-background">
        ...
        />
        <div className="flex min-h-0 flex-1 overflow-y-auto overscroll-contain touch-pan-y p-4 md:p-6">
  ...
  return (
    <>
      <div className="overscroll-behavior-contain flex h-dvh min-w-0 touch-pan-y bg-background">
        <div className="flex min-w-0 flex-1 flex-col">
          ...
          <div className="sticky bottom-0 z-1 mx-auto flex w-full max-w-4xl gap-2 border-t-0 bg-background/70 px-2 pb-3 md:px-4 md:pb-5 backdrop-blur-sm">
            {!isReadonly && inputElement}
          </div>
        </div>
        <ToolActivityRail messages={messages} />
      </div>
    </>
  );
}
