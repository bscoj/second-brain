...
const clientPort = Number.parseInt(
  process.env.CHAT_APP_CLIENT_PORT || '3002',
  10,
);
const PORT = Number.parseInt(
  process.env.CHAT_APP_SERVER_PORT ||
    process.env.CHAT_APP_PORT ||
    process.env.PORT ||
    (isDevelopment ? '3001' : '3002'),
  10,
);
const corsOrigin =
  process.env.CHAT_APP_CORS_ORIGIN ||
  (isDevelopment ? `http://localhost:${clientPort}` : true);
